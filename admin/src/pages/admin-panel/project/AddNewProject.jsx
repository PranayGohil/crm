...
const AddNewProject = () => {
  ...
  const updateItem = (i, field, val) => {
    const u = [...items];
    u[i][field] = field === "name" ? val : Number(val);
    setItems(u);
  };
  const addItem = () =>
  ...
  const resetRow = (i) =>
    setItems(
      items.map((it, j) =>
        j === i ? { name: "", quantity: 0, price: 0 } : it,
      ),
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3 sm:p-6">
      ...
      <form
        ...
        className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 space-y-5"
      >
        {/* Project Name */}
        <div>
          ...
          {touched.project_name && errors.project_name && (
            <p className="text-red-600 text-xs mt-1">{errors.project_name}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {touched.assign_date && errors.assign_date && (
              <p className="text-red-600 text-xs mt-1">{errors.assign_date}</p>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-xl">
          <div>
            ...
            <SearchableSelect
              ...
              options={["INR", "USD", "EUR", "AED"].map((c) => ({ value: c, label: c }))}
              isClearable={false}
            />
          </div>
          <div>
            <label className={labelCls}>Subtotal</label>
            <div className="px-3 py-2 text-sm bg-white border border-gray-300 rounded-lg text-gray-700">
              {getSubTotal()}
            </div>
          </div>
          <div>
        ...
        {selectedFiles.length > 0 && (
          <div>
            ...
            <p className="text-xs text-gray-500">
              ...
              ·{" "}
              {(
                selectedFiles.reduce((a, f) => a + f.size, 0) /
                1024 /
                1024
              ).toFixed(2)}{" "}
              MB total
        ...
        {/* Upload area */}
        <div
          className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-blue-400 hover:bg-blue-50 transition-colors"
          onDragOver={(e) => {
            e.preventDefault();
