...
const EditClient = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      username: Yup.string().matches(/^[a-zA-Z0-9_-]+$/, { message: "Letters, numbers, _ and - only", excludeEmptyString: true }).required("Username is required"),
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        setLoading(true);
        await axios.put(`${process.env.REACT_APP_API_URL}/api/client/update-username/${id}`, values, {
    ...
    const fetchClient = async () => {
      try {
        ...
        const mergedStagePricing = STAGE_OPTIONS.map((stage) => {
          ...
          return { stage_name: stage, price: existing?.price ?? 0 };
        });
        formik.setValues({
          full_name: client.full_name || "", email: client.email || "", phone: client.phone || "",
          joining_date: client.joining_date ? client.joining_date.substring(0, 10) : "",
          address: client.address || "", username: client.username || "", password: client.password || "",
          client_type: client.client_type || "", company_name: client.company_name || "",
          gst_number: client.gst_number || "", business_phone: client.business_phone || "",
          website: client.website || "", linkedin: client.linkedin || "",
          business_address: client.business_address || "", additional_notes: client.additional_notes || "",
          stages: (client.stages && client.stages.length > 0) ? client.stages : STAGE_OPTIONS,
    ...
    <div className="min-h-screen bg-gray-50 p-3 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-4 sm:mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg sm:text-2xl font-semibold text-gray-800">Edit Client</h1>
            <p className="text-xs sm:text-sm text-gray-500">Update client details</p>
          </div>
        </div>
      </div>
              ...
              <div className="sm:col-span-2">
                <Field label="Password" error={touched.password && errors.password}>
                  <div className="relative">
                    <input type={showPassword ? "text" : "password"} name="password" value={values.password} onChange={handleChange} placeholder="Password" disabled={isSubmitting} className={inputCls} />
          ...
          <section className="pt-6 border-t border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <Field label="Company Name">
                <input type="text" name="company_name" value={values.company_name} onChange={handleChange} disabled={isSubmitting} className={inputCls} />
              </Field>
              <Field label="GST / VAT Number">
                <input type="text" name="gst_number" value={values.gst_number} onChange={handleChange} disabled={isSubmitting} className={inputCls} />
              </Field>
              <Field label="Business Phone">
                <input type="text" name="business_phone" value={values.business_phone} onChange={handleChange} disabled={isSubmitting} className={inputCls} />
              </Field>
              <Field label="Website">
                <input type="text" name="website" value={values.website} onChange={handleChange} disabled={isSubmitting} className={inputCls} />
              </Field>
              <div className="sm:col-span-2">
                <Field label="LinkedIn">
                  <input type="text" name="linkedin" value={values.linkedin} onChange={handleChange} disabled={isSubmitting} className={inputCls} />
                </Field>
              </div>
              <div className="sm:col-span-2">
                <Field label="Business Address">
                  <input type="text" name="business_address" value={values.business_address} onChange={handleChange} disabled={isSubmitting} className={inputCls} />
                </Field>
              </div>
              <div className="sm:col-span-2">
                <Field label="Additional Notes">
                  <input type="text" name="additional_notes" value={values.additional_notes} onChange={handleChange} disabled={isSubmitting} className={inputCls} />
                </Field>
              </div>
            </div>
          </section>
