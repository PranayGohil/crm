    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {/* ── NEW: Total Earned across all clients ── */}
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  ₹{clients
                    ...
                    .reduce((sum, s) => sum + (s.earned_amount || 0), 0)
                    .toLocaleString()}
                </p>
              </div>
              <div className="w-10 h-10 flex items-center justify-center rounded-full bg-white text-green-600 font-bold text-lg">
                ₹
              </div>
            </div>
          </div>

          {/* ── NEW: Total Pending across all clients ── */}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  ₹{clients
                    .flatMap((c) => c.subtasks || [])
                    .reduce((sum, s) => sum + ((s.total_price || 0) - (s.earned_amount || 0)), 0)
                    .toLocaleString()}
                </p>
              </div>
              <div className="w-10 h-10 flex items-center justify-center rounded-full bg-white text-yellow-600 font-bold text-lg">
                ₹
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {error ? (
          ...
          filteredClients.map((client) => {
            ...
            // ── Earnings computed from subtask fields ──
            ...
            return (
              <div key={client._id || client.username}
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center text-sm text-gray-600 mb-4">
                    ...
                    <span>Joined: {client.joining_date ? new Date(client.joining_date).toLocaleDateString() : "N/A"}</span>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="text-gray-600">
                        Tasks: <span className="font-medium">{done}</span> / <span className="font-medium">{total}</span> Completed
                      </span>
                      <span className="text-green-600 font-medium">{completedPercent}%</span>
                    </div>
                    ...
                    <div className="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-2">
                        <div className="text-center">
                          ...
                          <p className="text-sm font-semibold text-gray-800">₹{totalValue.toLocaleString()}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-xs text-green-600">Earned</p>
                          <p className="text-sm font-semibold text-green-700">₹{earnedValue.toLocaleString()}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-xs text-yellow-600">Pending</p>
                          <p className="text-sm font-semibold text-yellow-700">₹{pendingValue.toLocaleString()}</p>
                        </div>
                      </div>
                      {/* Earning progress bar */}
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${earningPercent}%` }}></div>
                      </div>
