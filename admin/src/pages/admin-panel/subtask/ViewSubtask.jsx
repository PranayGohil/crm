  ...
  useEffect(() => {
    ...
    socket.on("subtask_updated", (data) => {
      if (data._id === subtaskId) {
        setSubtask((prev) => ({ ...prev, status: data.status, priority: data.priority }));
      }
    });
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="flex justify-between bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div className="flex items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold text-gray-800">{project?.project_name || "Project Name"}</h1>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-xl font-semibold text-gray-800">{subtask.task_name || "Subtask Name"}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{assignedEmployee?.full_name || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Start Date:</span>
                  <span className="font-medium">{formatDate(subtask.assign_date)}</span>
                </div>
              </div>
              <div className="space-y-3">
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatDate(subtask.due_date)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              ...
              <p className="text-gray-700">{subtask.description || "No description available."}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Attached Media</h2>
                  ...
                  <div key={index} className="relative group">
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-lg">
                      <div className="flex space-x-2">
                        ...
                        <button onClick={() => { setMediaToRemove(item.src); setShowRemoveModal(true); }}
                          className="p-2 bg-white rounded-full hover:bg-gray-100">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="mb-6">
              <div className="flex items-start space-x-3">
                <img src={profilePic} alt="Your profile" className="w-10 h-10 rounded-full object-cover" />
                  ...
                  <div key={comment._id} className="flex items-start space-x-3">
                    {comment.user_type === "admin" ? (
                      <>
                        ...
                        <img src={profilePic} alt="Admin" className="w-10 h-10 rounded-full object-cover" />
                      </>
                    ) : (
                      <>
                        ...
                        <div className="flex-1 bg-gray-50 p-4 rounded-lg">
                          <div className="flex justify-between items-center mb-2">
                            <span className="font-medium">{comment.user_id?.full_name || "Unknown User"}</span>
                            <span className="text-sm text-gray-500">{formatDate(comment.created_at)}</span>
                          </div>
                          <p className="text-gray-700">{comment.text}</p>
                        </div>
        ...
        <div className="space-y-6">
          ...
          {assignedEmployee && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">Assigned To</h3>
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{assignedEmployee.email}</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                <div className="bg-gray-50 rounded-lg p-3 text-center">
                  ...
                  <p className="text-sm font-bold text-gray-800">₹{totalPrice.toLocaleString()}</p>
                </div>
                <div className="bg-green-50 rounded-lg p-3 text-center">
                  <p className="text-xs text-green-600 mb-1">Earned</p>
                  <p className="text-sm font-bold text-green-700">₹{earnedAmount.toLocaleString()}</p>
                </div>
                <div className="bg-yellow-50 rounded-lg p-3 text-center">
                  <p className="text-xs text-yellow-600 mb-1">Pending</p>
                  <p className="text-sm font-bold text-yellow-700">₹{pendingAmount.toLocaleString()}</p>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-blue-500 h-2 rounded-full transition-all"
                    style={{ width: `${earningPercent}%` }}></div>
                </div>
              </div>

              {/* ── Stage-wise price breakdown ── */}
              <div>
                ...
                <div className="space-y-2">
                  {subtask.stages?.map((stage, i) => {
                    ...
                    return (
                      <div key={i}
                        className={`flex items-center justify-between p-2 rounded-lg ${completed ? "bg-green-50 border border-green-100" : "bg-gray-50 border border-gray-100"
                          }`}>
                        <div className="flex items-center gap-2">
                          {completed ? (
                          ...
                          <span className={`text-sm ${completed ? "text-green-800 font-medium" : "text-gray-600"}`}>
                            {name}
                          </span>
                        </div>
                        <span className={`text-sm font-semibold ${completed ? "text-green-700" : "text-gray-500"}`}>
