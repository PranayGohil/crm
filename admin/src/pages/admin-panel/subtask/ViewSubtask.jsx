...
const ViewSubtask = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setMediaItems(
        (subtaskData.media_files || []).map((file) => ({
          src: file,
          alt: file,
        })),
      );
      ...
      const { data: projectData } = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/project/get/${subtaskData.project_id._id}`,
        { headers },
      );
  ...
  useEffect(() => {
    ...
    socket.on("comment", (data) => {
      if (data.related_id === subtaskId) fetchData();
    });
  ...
  const earningPercent =
    totalPrice > 0 ? Math.round((earnedAmount / totalPrice) * 100) : 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3 sm:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="w-40">
                    <SearchableSelect
                      ...
                      value={
                        editingStatus
                          ? { value: editingStatus, label: editingStatus }
                          : null
                      }
                  ...
                  <div className="w-40">
                    <SearchableSelect
                      ...
                      value={
                        editingPriority
                          ? { value: editingPriority, label: editingPriority }
                          : null
                      }
                      onChange={(opt) =>
                        setEditingPriority(opt ? opt.value : "")
                      }
        ...
        <div
          className={`space-y-4 sm:space-y-6 ${showSidebar ? "block" : "hidden lg:block"}`}
        >
          {/* Mobile close button */}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                ...
                <div className="bg-yellow-50 rounded-lg p-2 text-center">
                  ...
                  <p className="text-xs sm:text-sm font-bold text-yellow-700">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <span
                            className={`text-xs truncate ${completed ? "text-green-800 font-medium" : "text-gray-600"}`}
                          >
                            {name}
                          </span>
                        </div>
                        <span
                          className={`text-xs font-semibold flex-shrink-0 ml-1 ${completed ? "text-green-700" : "text-gray-500"}`}
                        >
