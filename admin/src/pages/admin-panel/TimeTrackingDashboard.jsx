      ...
      {stages.map((s, i) => {
        ...
        return (
          <React.Fragment key={i}>
            <span
              className={`px-2 py-0.5 text-xs rounded-full border ${done ? "bg-green-50 text-green-700 border-green-200" : "bg-gray-50 text-gray-600 border-gray-200"}`}
            >
              {done && "✓ "}
              {name}
            </span>
        ...
        <div className="space-y-4 mb-6">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setCustomDates((p) => ({ ...p, from: e.target.value }))
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setCustomDates((p) => ({ ...p, to: e.target.value }))
              }
              className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            />
          </div>
...
const TimeTrackingDashboard = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API}/api/employee/get-all`, { headers })
      .then((res) => {
    ...
    async (page = 1, limit = pagination.limit) => {
      ...
      try {
        const params = new URLSearchParams({
          ...
          ...(debouncedSubtaskSearch && {
            subtaskSearch: debouncedSubtaskSearch,
          }),
          ...(selectedEmployee && { employee: selectedEmployee }),
          ...(selectedRange === "custom" &&
            customDates.from && { from: customDates.from }),
          ...(selectedRange === "custom" &&
            customDates.to && { to: customDates.to }),
        });
    ...
    <div className="min-h-screen bg-gray-50 p-3 sm:p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
        {[
          ...
          <div
            key={label}
            className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-5 text-center"
          >
            <p className="text-base sm:text-2xl font-bold text-gray-800 leading-tight">
              {value}
            </p>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <button
              ...
              onClick={() =>
                setOpenProject((p) => (p === project._id ? null : project._id))
              }
            >
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3">
                              <span
                                className="block max-w-[200px] truncate text-gray-800 font-medium"
                            ...
                            <td className="px-4 py-3">
                              <span
                                className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full ${remainingCls[type]}`}
                              >
                            ...
                            <td className="px-4 py-3">
                              <Link
                                to={`/subtask/logs/${subtask._id}`}
                                className="text-blue-600 hover:text-blue-800 flex"
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-2 text-xs">
                          ...
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${remainingCls[type]}`}
                          >
                            <span className="w-1.5 h-1.5 rounded-full bg-current opacity-60" />
                            {label}
                          </span>
              ...
              {Array.from({ length: pagination.totalPages }, (_, i) => i + 1)
                ...
                .map((p, i) =>
                  p === "…" ? (
                    ...
                    <button
                      key={p}
                      onClick={() => fetchData(p, pagination.limit)}
                      disabled={loading}
              ...
              <div className="w-28 ml-1">
                <SearchableSelect
                  value={{
                    ...
                    label: `${pagination.limit}/page`,
                  }}
                  onChange={(opt) => fetchData(1, Number(opt.value))}
